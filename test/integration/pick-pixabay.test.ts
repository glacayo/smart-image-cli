...
const jpeg = (w: number, h: number, c = "#cde") =>
  sharp({ create: { width: w, height: h, channels: 3, background: c } }).jpeg().toBuffer();
const hit = (p: Partial<PixabaySearchHit> & Pick<PixabaySearchHit, "id">): PixabaySearchHit => ({
  pageURL: `https://pixabay.com/photos/x-${p.id}/`, webformatURL: `https://cdn.example/w-${p.id}.jpg`,
  largeImageURL: `https://cdn.example/l-${p.id}.jpg`, imageWidth: 2000, imageHeight: 1333, user: "alice", ...p
});
...
describe("pickService --source pixabay (WU5b3)", () => {
  it("transaction: success/cache/one-dl/license/used-id/allow-reuse/cap/rollbacks/errors", async () => {
    ...
    const hits = [
      ...
      hit({ id: 101, user: "dup" }), hit({ id: 202, user: "bob", imageWidth: 2400, imageHeight: 1600 }),
    ...
    const opts = {
      source: "pixabay" as const, query: "kitchen remodel", orientation: "landscape" as const,
      width: 1200, height: 800, slot: "hero", location: "home", safeSearch: true
    };
    ...
    expect(search.mock.calls[0]?.[0]).toMatchObject({
      query: "kitchen remodel", safesearch: true, orientation: "horizontal"
    });
    const m = man(first);
    expect(m).toMatchObject({
      source: "pixabay", pixabayId: 101, pageURL: "https://pixabay.com/photos/kitchen-101/",
      contributor: "alice", license: "Pixabay Content License", cache: "miss"
    });
    ...
    const usedTxt = await fs.readFile(path.join(root, ".img-ia", "pixabay", "used-ids.jsonl"), "utf8");
    expect(usedTxt).toContain(`"id":101`);
    expect(usedTxt + (await fs.readFile(path.join(root, ".img-ia", "usage.jsonl"), "utf8"))).toContain(String(m.sha256));
    expect(JSON.stringify(first.result)).not.toContain(SECRET);

    const capDl = vi.fn(async () => jpeg(1280, 960, "#123"));
    const cap = await pickService(await tmp(), {
      source: "pixabay", query: "wide", width: 1600, height: 1200, slot: "b", location: "t"
    }, { pixabayClient: {
      search: vi.fn(async () => ({ hits: [hit({ id: 404, imageWidth: 4000, imageHeight: 3000 })], total: 1 })),
      download: capDl
    } });
    expect(cap.exitCode).toBe(0);
    expect(capDl).toHaveBeenCalledTimes(1);
    expect(man(cap).warnings).toEqual(expect.arrayContaining([
    ...
    const failed = await pickService(failRoot, {
      ...
      index: { recordUsageEvent: () => { throw new Error("sqlite boom"); }, close: () => undefined } as never
    });
    ...
    const idxFail = await pickService(idxR, {
      ...
      usedIds: {
        ...
        append: async () => { throw new Error(`used-id boom ${SECRET}`); }
      }
    });
    expect(idxFail.exitCode).toBe(0);
    const im = man(idxFail);
    expect(im.warnings).toEqual(expect.arrayContaining([
    ...
    roots.push(home);
    vi.stubEnv("APPDATA", home); vi.stubEnv("XDG_CONFIG_HOME", home);
    ...
    const rate = await pickService(root, { source: "pixabay", query: "k" }, { pixabayClient: {
      search: vi.fn(async () => {
        throw new PixabayClientError("rate_limited", "Pixabay rate limit exceeded", {
          status: 429, rateLimit: { limit: 100, remaining: 0, resetSeconds: 30 }
        });
      }),
      download: vi.fn(async () => { throw new Error("x"); })
    } });
    ...
    const base = {
      source: "pixabay" as const, query: "cached kitchen", orientation: "landscape" as const,
      width: 800, height: 500, slot: "a", location: "b"
    };
    expect((await pickService(cRoot, base, { pixabayClient: { search: s2, download: d2 } })).exitCode).toBe(0);
    const c2 = await pickService(cRoot, { ...base, slot: "c", location: "d" }, {
    ...
    const dl = vi.fn(async () => { throw new Error("no"); });
    for (const x of [
      { o: { source: "pixabay" as const, query: "n", width: 2000, height: 1500 }, hits: [hit({ id: 9, imageWidth: 100, imageHeight: 80 })] },
      { o: { source: "pixabay" as const, query: "z" }, hits: [] as PixabaySearchHit[] },
      { o: { source: "pixabay" as const, query: "p", orientation: "portrait" as const, width: 100, height: 200 }, hits: [hit({ id: 11, imageWidth: 2000, imageHeight: 1000 })] }
    ]) {
      expect(await pickService(root, x.o, {
        pixabayClient: { search: vi.fn(async () => ({ hits: x.hits, total: x.hits.length })), download: dl }
